...
class Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.opt.input_coco_det_json != '':
            with open(self.opt.input_coco_det_json, 'rb') as f:
                self.coco_det_rlts = json.load(f)
            self.map_id2index = dict()
            for index in range(len(self.coco_det_rlts)):
                self.map_id2index[self.coco_det_rlts[index]['image_id']] = index
            self.det_rlts_list = list()
            for ix in range(len(self.info['images'])):
                # ix -> id -> index -> det_rlts
                id = self.info['images'][ix]['id']
                index = self.map_id2index[id]
                self.det_rlts_list.append(self.coco_det_rlts[index]['category_ids'])



        # separate out indexes for each of the provided splits
        random.seed(opt.random_seed)
        self.split_ix = {'paired_train': [], 'unpaired_images_train': [], 'unpaired_captions_train': [],
                         'train': [], 'val': [], 'test': []}
        for ix in range(len(self.info['images'])):
            ...
            if not 'split' in img:
                ...
                self.split_ix['train'].append(ix)
        paired_train_ix = random.sample(self.split_ix['train'], int(len(self.split_ix['train'])*opt.paired_percentage + 0.5))
        for ix in paired_train_ix:
            self.split_ix['paired_train'].append(ix)
        unpaired_train_ix = list(set(self.split_ix['train']) - set(paired_train_ix))
        for ix in unpaired_train_ix:
            self.split_ix['unpaired_images_train'].append(ix)
            self.split_ix['unpaired_captions_train'].append(ix)

        print('assigned %d images to split paired_train' % len(self.split_ix['paired_train']))
        print('assigned %d images to split unpaired_images_train' % len(self.split_ix['unpaired_images_train']))
        print('assigned %d images to split unpaired_captions_train' % len(self.split_ix['unpaired_captions_train']))
    ...
    def collate_func(self, batch, split):
        ...
        for sample in batch:
            ...
            tmp_fc, tmp_att, tmp_seq, \
                ix, it_pos_now, tmp_wrapped, tmp_dets = sample
            if tmp_wrapped:
                wrapped = True
        ...
        fc_batch, att_batch, label_batch, gts, infos, dets = \
            zip(*sorted(zip(fc_batch, att_batch, label_batch, gts, infos, dets), key=lambda x: 0, reverse=True))
        data = {}
    ...
    def __getitem__(self, index):
        ...
        if hasattr(self, 'h5_label_file'):
            ...
            seq = None
        if hasattr(self, 'det_rlts_list'):
            dets = self.det_rlts_list[ix]
        else:
            dets = None
        return (fc_feat,
                att_feat, seq,
                ix, it_pos_now, wrapped, dets)
...
class DataLoader:
    def __init__(self, opt):
        ...
        for split in ['paired_train', 'unpaired_images_train', 'unpaired_captions_train', 'train', 'val', 'test']:
            if split in ['paired_train', 'unpaired_images_train', 'unpaired_captions_train', 'train']:
