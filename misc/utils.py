...
class DistillLanguageModelCriterion(nn.Module):
    def __init__(self):
        super(DistillLanguageModelCriterion, self).__init__()
        self.criterion = torch.nn.KLDivLoss(reduction='sum')

    def forward(self, input, target, mask, score_logit_weighting):
        if target.ndim == 3:
            target = target.reshape(-1, target.shape[2])
            # mask = mask.reshape(-1, mask.shape[2])
            mask = mask.reshape(input.shape[0], -1, mask.shape[3])
        temperature = 1
        # mask = F.softmax(mask, dim=-1)
        # truncate to the same size
        target = target[:, :input.size(1)]
        mask = mask[:, :input.size(1)]
        input = input[:, :input.size(1)]
        ...
        used_bool = target > 0
        # used_index = used_bool.float()

        # output = -input.gather(2, target.unsqueeze(2)).squeeze(2) * mask
        # # Average over each token
        # output = torch.sum(output) / torch.sum(mask)

        # output = (self.criterion(input, mask) - self.criterion(mask, mask))/ target.size(0) / target.size(1)
        model_ensemble_prob = F.softmax(score_logit_weighting, -1).view(-1).unsqueeze(-1).unsqueeze(-1)
        # input = input[used_bool]
        # mask = mask[used_bool]
        mse = 0.5 * (((input - mask)/temperature)**2)
        mse *= model_ensemble_prob
        output = mse[used_bool].mean() * score_logit_weighting.shape[1]
        # output = -(mask*input).sum()/target.size(0) / target.size(1)
...
class gradient_descent:
    def __init__(self, x, stepsize=1e-3, alpha=0.9):
        self.historical_grad = x.new_zeros(x.shape)
        self.stepsize = stepsize
        self.alpha = alpha
        self.iter = 0

    def update(self, x):
        self.historical_grad = self.alpha * self.historical_grad + (1 - self.alpha) * x.grad.data
        x.data.add_(-self.stepsize, x.grad.data)
        x.grad.data *= 0
        # return x

class gradient_descent_adagrad:
    def __init__(self, x, stepsize=1e-3, alpha=0.9, eps=1e-8):
        self.historical_grad_square = x.new_zeros(x.shape)
        self.stepsize = stepsize
        self.alpha = alpha
        self.iter = 0
        self.eps = eps
    ...
    def update(self, x):
        if self.iter == 0:
            self.historical_grad_square += x.grad.data ** 2
        else:
            self.historical_grad_square = self.alpha * self.historical_grad_square + (1 - self.alpha) * x.grad.data ** 2
        adj_grad = x.grad.data / torch.sqrt(self.historical_grad_square + self.eps)
        x.data.add_(-self.stepsize, adj_grad)
        x.grad.data *= 0
        self.iter += 1
