...
class KDLossWrapper(torch.nn.Module):
    def __init__(self, model, opt):
        super(KDLossWrapper, self).__init__()
        self.opt = opt
        self.model = model
        if opt.label_smoothing > 0:
            self.crit = utils.LabelSmoothing(smoothing=opt.label_smoothing)
        else:
            self.crit = utils.DistillLanguageModelCriterion()
        self.rl_crit = utils.RewardCriterion()
        self.struc_crit = utils.StructureLosses(opt)

    def forward(self, fc_feats, att_feats, labels, masks, att_masks, gts, gt_indices,
                sc_flag, struc_flag, score_logit_weighting):
        opt = self.opt
        
        out = {}
        if struc_flag:
            if opt.structure_loss_weight < 1:
                lm_loss = self.crit(self.model(fc_feats, att_feats, labels, att_masks), labels[..., 1:], torch.narrow(masks, masks.dim()-2, 1, masks.shape[-2]-1), score_logit_weighting)
            ...
            loss = self.crit(self.model(fc_feats, att_feats, labels, att_masks), labels[..., 1:], torch.narrow(masks, masks.dim()-2, 1, masks.shape[-2]-1), score_logit_weighting)
        else:
            self.model.eval()
            with torch.no_grad():
